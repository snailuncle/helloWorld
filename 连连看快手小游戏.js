...
function lianLianKan(){
    ...
    function penApple(p1, p2) {
        // if(!(rowExtract(p1)==3 && columnExtract(p1)==5 && rowExtract(p2)==10 && columnExtract(p2)==7)){return;}
    ...
    // =====================================================
        ...
        // 第一种两个图片在同一行
        if (onlineX(p1, p2)) {
            ////////////////////////////////////log("177行出入的应该是同一行,if(onlineX(p1, p2))参数是",p1,p2)
            // 两个图片挨着
            if (p1NextToP2("onlineX", p1, p2)) {
                ////////////////////////////////////log("132行p1,p2在一起",p1,p2)
                return true;
            } else if (obstacleIsThereBetweenP1AndP2("onlineX", p1, p2)) {
                ////////////////////////////////////////log("136行图片中有障碍物else if (obstacleIsThereBetweenP1AndP2(onlineX, p1, p2)",p1,p2)
                // exit()
                // p1,p2中间有障碍物
                //那么p1的下面p3是不是空的?
                ...
                // let rowsNumber = 10;
                // let columnsNumber = 7;
                for (let k = 1; k <= (rowsNumber - rowExtract(p1)); k++) {
    ...
    function is2ImgThoroughfareZ(p1, p2) {
        ...
        if (!isImgBackground(p3) || !isImgBackground(p4)) {
            ...
            //p4和p5不能有障碍物
            //循环次数为p2-p1-3
            let cycleTimes=rowExtract(p2)-rowExtract(p1)-3
            for (let k = 1; k <= cycleTimes; k++) {

                let p3 = imgEntityExtractFromRowAndcolumn(rowExtract(p1), columnExtract(p1)+1+k);
                let p3new = new ImgUnit(p3, [rowExtract(p1), columnExtract(p1)+1+k]);
    ...
    function is2ImgThoroughfareZ2(p1, p2) {
        ...
        if (!isImgBackground(p3) || !isImgBackground(p4)) {
            ...
            for (let k = 1; k <= cycleTimes; k++) {
                ...
                let p3 = imgEntityExtractFromRowAndcolumn(rowExtract(p1)+1+k, columnExtract(p1));
                let p3new = new ImgUnit(p3, [rowExtract(p1)+1+k, columnExtract(p1)]);
    ...
    function is2ImgThoroughfareZ3(p1, p2) {
        ...
        if (!isImgBackground(p3) || !isImgBackground(p4)) {
            ...
            //p4和p5不能有障碍物
            //循环次数为p2-p1-3
            let cycleTimes=columnExtract(p2)-columnExtract(p1)-3
            for (let k = 1; k <= cycleTimes; k++) {
    ...
    function imgEntityExtractFromRowAndcolumn(row, column) {
        ////////////////////////////////////////////////////////////log("424行imgEntityExtractFromRowAndcolumn(row,column)的参数是",row,column)
